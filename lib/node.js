...
function Node(options) {
  ...
  this._rpc = new options.transport(options);
...
Node.prototype._putValidatedKeyValue = function(key, value, callback) {
  ...
  var message = new Message({
    method: 'STORE',
    params: { item: item, contact: this._self }
  });
...
Node.prototype._handleStore = function(incomingMsg) {
  ...
  try {
    item = new Item(params.key, params.value, params.contact.nodeID);
  } catch(err) {
    return;
  }
  ...
  this._validateKeyValuePair(item.key, params.value, function(valid) {
    if (!valid) {
...
Node.prototype._handleFindValue = function(incomingMsg) {
  ...
  this._storage.get(params.key, function(err, value) {
    ...
    var foundMessage = new Message({
      id: incomingMsg.id,
      result: { value: value, contact: node._self }
    });
