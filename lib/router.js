...
Router.prototype.lookup = function(type, key, callback) {
  ...
  state.message = new Message({
    method: 'FIND_' + type,
    params: { key: key, contact: this._self }
  });
...
Router.prototype._queryContact = function(state, contactInfo, callback) {
  ...
  this._rpc.send(contact, state.message, function(err, message) {
    if (err) {
      self._removeFromShortList(state, contact.nodeID);
      return callback();
    }

    self._handleFindResult(state, message, contact, callback);
  });
};
...
Router.prototype._validateFindResult = function(state, msg, contact, done) {
  ...
  try {
    parsedValue = JSON.parse(msg.result.value).value;
  } catch(err) {
    this._log.warn('failed to parse value %s', msg.result.item);
...
Router.prototype._handleValueReturned = function(state, callback) {
  ...
  if (distances.length >= 1) {
    ...
    var message = new Message({
      method: 'STORE',
      params: { key: state.key, value: state.value, contact: this._self}
    });
