...
* #_handleFindResult
* @param {object} state
* @param {object} data - message response params
* @param {object} contact
* @param {function} callback
*/
Router.prototype._handleFindResult = function(state, data, contact, callback) {
  ...
  if (state.type === 'NODE') {
    this._addToShortList(state, data.contacts);
    return callback();
  }
  ...
  this._validateFindResult(state, data, contact, callback);
};
 ...
 * @param {function} callback
 */
Router.prototype._validateFindResult = function(state, data, contact, done) {
  var self = this;
  var parsedValue;

  try {
    parsedValue = JSON.parse(data.value).value;
  } catch(err) {
    this._log.warn('failed to parse value %s', data.value);
